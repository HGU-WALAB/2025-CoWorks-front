...
import { FolderCreateModalProps } from '../types/folder';
...
const FolderCreateModal: React.FC<FolderCreateModalProps> = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (folderName.trim().length > 100) {
      setError('폴더 이름은 100자 이내로 입력해주세요.');
      return;
    }
                  ...
                  <div className="mt-2">
                    {parentFolderName && (
                      <p className="text-sm text-gray-500 mb-4">
                        상위 폴더: <span className="font-medium">{parentFolderName}</span>
                      </p>
                    )}
